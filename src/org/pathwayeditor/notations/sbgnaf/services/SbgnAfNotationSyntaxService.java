...
//	"{ x y w h 0.2 w mul 0.20 h mul rrect } ifelse";
	...
	// shapes
	private ShapeObjectType Activity;
...
//	private LinkObjectType Catalysis;
	...
	public SbgnAfNotationSyntaxService(INotationSubsystem serviceProvider) {
		...
		createRMO();
		// shapes
		this.Activity = new ShapeObjectType(this, 100, "Activity");
		createActivity();
...
//		createProduction();
		this.Modulation = new LinkObjectType(this, 22, "Unknown influence");
		createModulation();
		this.Stimulation = new LinkObjectType(this, 23, "Positive influence");
		createStimulation();
//		this.Catalysis = new LinkObjectType(this, 24, "Catalysis");
//		createCatalysis();
		this.Inhibition = new LinkObjectType(this, 25, "Negative influence");
		createInhibition();
		...
		// shape set
		this.shapes.put(this.Activity.getUniqueId(), this.Activity);
...
//	}
	...
	public ShapeObjectType getActivity() {
...
//
	...
	private void createActivity() {
		this.Activity.setDescription("Biological activity");// ment to be
																	// TypeDescription
																	// rather
		this.Activity.getDefaultAttributes().addPropertyDefinition(createNameProperty());
		this.Activity.getDefaultAttributes().setShapeDefinition(UNIT_OF_INFO_DEFN);
		this.Activity.getDefaultAttributes().setFillColour(RGB.WHITE);
		this.Activity.getDefaultAttributes().setSize(new Dimension(60, 40));
		this.Activity.getDefaultAttributes().setLineWidth(1);
		this.Activity.getDefaultAttributes().setLineStyle(LineStyle.SOLID);
		this.Activity.getDefaultAttributes().setLineColour(RGB.BLACK);
		EnumSet<EditableShapeAttributes> editableAttributes = EnumSet
				.noneOf(EditableShapeAttributes.class);
		if (true) {
			editableAttributes.add(EditableShapeAttributes.FILL_COLOUR);
		...
		this.Activity.setEditableAttributes(editableAttributes);
	}

	private void defineParentingActivity() {
		HashSet<IShapeObjectType> set = new HashSet<IShapeObjectType>();
		set.addAll(Arrays.asList(new IShapeObjectType[] { this.Complex,
				this.GeneticUnit, this.Macromolecule, this.SimpleChem,
				this.UnspecEntity}));
		for (IShapeObjectType child : set) {
			this.Activity.getParentingRules().addChild(child);
		}
...
//	}
	...
	private void createComplex() {
		...
		this.Complex.getDefaultAttributes().setSize(new Dimension(30, 20));
	...
	private void defineParentingComplex() {
		this.Complex.getParentingRules().clear();
	...
	private void defineParentingGeneticUnit() {
		this.GeneticUnit.getParentingRules().clear();
	...
	private void createMacromolecule() {
		...
		this.Macromolecule.getDefaultAttributes().setSize(new Dimension(30, 20));
	...
	private void defineParentingMacromolecule() {
		this.Macromolecule.getParentingRules().clear();
	...
	private void defineParentingSimpleChem() {
		this.SimpleChem.getParentingRules().clear();
	...
	private void defineParentingUnspecEntity() {
		this.UnspecEntity.getParentingRules().clear();
...
//	}
	...
	private void createModulation() {
		Set<IShapeObjectType> set = null;
		this.Modulation
				.setDescription("An unknown influenceis used when the effect exerted from one activity to another is not well understood.");
		int[] lc = new int[] { 0, 0, 0 };
		...
		set = new HashSet<IShapeObjectType>();
		set.addAll(Arrays.asList(new IShapeObjectType[] { this.Activity, this.Observable }));
		for (IShapeObjectType tgt : set) {
	...
	private void createStimulation() {
		Set<IShapeObjectType> set = null;
		this.Stimulation
				.setDescription("A positive influence produce positive or activating effect from one activity to another.");
		int[] lc = new int[] { 0, 0, 0 };
		...
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.Perturbation, this.Activity);
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.Perturbation, this.Observable);
//		for (IShapeObjectType tgt : set) {
//			this.Stimulation.getLinkConnectionRules().addConnection(
//					this.Perturbation, tgt);
//		}
		...
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.Activity, this.Observable);
//		for (IShapeObjectType tgt : set) {
//			this.Stimulation.getLinkConnectionRules().addConnection(
//					this.Activity, tgt);
//		}
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.AndGate, this.Activity);
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.AndGate, this.Observable);
//		for (IShapeObjectType tgt : set) {
//			this.Stimulation.getLinkConnectionRules().addConnection(
//					this.AndGate, tgt);
//		}
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.OrGate, this.Activity);
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.OrGate, this.Observable);
//		for (IShapeObjectType tgt : set) {
//			this.Stimulation.getLinkConnectionRules().addConnection(
//					this.OrGate, tgt);
//		}
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.NotGate, this.Activity);
		this.Stimulation.getLinkConnectionRules().addConnection(
				this.NotGate, this.Observable);
//		for (IShapeObjectType tgt : set) {
//			this.Stimulation.getLinkConnectionRules().addConnection(
//					this.NotGate, tgt);
//		}
